...
/**
 * Performance test for the Prepayment entity.
 */
class PrepaymentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Prepayment entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all prepayments")
            .get("/api/prepayments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new prepayment")
            .post("/api/prepayments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "prepaymentAccount":"SAMPLE_TEXT"
                , "prepaymentAccountNumber":"SAMPLE_TEXT"
                , "narration":"SAMPLE_TEXT"
                , "remarks":"SAMPLE_TEXT"
                , "serviceOutletCode":"SAMPLE_TEXT"
                , "expenseAccount":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_prepayment_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created prepayment")
                .get("${new_prepayment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created prepayment")
            .delete("${new_prepayment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
